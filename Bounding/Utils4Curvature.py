...
def power_iteration(W1, W2=None, num_iters=50, return_vectors=False, verbose=False):
    ...
    if W2 is None:
        W2 = torch.ones(1, W1.shape[0], device=device)
    x = torch.randn((1, W1.shape[1]), device=device)
...
# def curvature_bound_general(model, c):
#     if model.nonlin == 'softplus':
#         g = 1.
#         h = 0.25
#     elif model.nonlin == 'sigmoid':
...
# def newton_step(x0, true_label, false_target, model, eps, verbose=True, device='cuda'):
...
#     eta = torch.zeros((batch_size, 1)).to(device)
#     eta_min = torch.zeros((batch_size, 1)).to(device)
#     eta_max = torch.zeros((batch_size, 1)).to(device)
...
# def newton_step_cert(x0, true_label, false_target, model, eps, verbose=True, device='cuda'):
#     batch_size = x0.shape[0]
#     x0 = x0.view(batch_size, -1)
#     input_dim = x0.shape[1]
...
#     # print(true_label)
#     print(M.shape, x0.shape, np.array(M))   
#     # print number of unique values in M
#     # print(torch.unique(M).shape)
#     # print(true_label[:5], false_target[:5])
#     # print(M[0:5])

#     raise

#     eta = torch.zeros((batch_size, 1)).to(device)
#     eta_min = -1/M*torch.zeros((batch_size, 1)).to(device)
#     eta_max = 1/m*torch.ones((batch_size, 1)).to(device)
#     eta = (eta_min + eta_max)/2.
...
#         targets = adversarial_targets(model, data, label, target_method='runner-up', device=device)
#         lower_bound, grad_norm = newton_step_cert(data, label, targets, model, eps, verbose=False, device=device)
#         logits = model(data)
...
#         f = adversarial_targets(model, data, label, target_method='runner-up', device=device)
#         data_attack = adversarial_attack(model, data, label, 0.01, eps, num_steps, 'L2', device=device)
...
#         target = adversarial_targets(model, data, label, 'runner-up', device=device)
#         if args.epsilon > 0:
#             data = newton_step(data, label, target, model, args.epsilon, device=device)
...
# def adversarial_targets(model, data, label, target_method, device='cuda'):
#     logits = model(data)
#     batch_size, num_classes = logits.shape
#     if target_method == 'runner-up':
#         preds = torch.zeros_like(logits).to(device)
#         preds[torch.arange(data.shape[0]), label] = 1e10
#         target_index = (logits - preds).argmax(dim=1)
#     elif target_method == 'random':
#         target_index = torch.randint(num_classes - 1, (batch_size,)).to(device)
...
# def adversarial_attack(model, x_natural, y, step_size, epsilon, perturb_steps, distance, device='cuda'):
#     batch_size = len(x_natural)
#     f = adversarial_targets(model, x_natural, y, target_method='runner-up', device=device)
#     x_adv = x_natural.detach() + 0.01 * torch.randn(x_natural.shape).to(device).detach()
